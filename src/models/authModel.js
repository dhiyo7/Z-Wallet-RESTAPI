...
module.exports = {
    ...
    authLogin: (body) => {
        return new Promise((resolve, reject) => {
            ...
            const queryStr = `SELECT id, email, password,is_active, pin FROM tb_user WHERE email = ?`
            db.query(queryStr, email, (err, data) => {
                if (err) {
                    ...
                    if (data.length < 1) {
                        reject({
                            status: 404,
                            message: `Username atau password salah!` //username ga ditemuin
                        })
                        ...
                        bcrypt.compare(password, data[0].password, (error, result) => {
                            ...
                            if (!result) {
                                reject({
                                    status: 404,
                                    msg: `Username atau Password salah!` //pasword salah
                                })
                            } else {
                                const payload = {
                                    id: data[0].id,
                                    ...
                                    if (!err) {
                                        if (data[0].pin == 0) {
                                            resolve({
                                                ...
                                                data: { ...payload, token } //ketahan suruh ke halaman set PIN
                                            })
                                        } else {
                                            resolve({
                                                status: 200,
                                                message: `Sukses Login`,
                                                data: { ...payload, token } //sukses ke home
                                            })
                                        }
                                    } else {
                                        reject({
